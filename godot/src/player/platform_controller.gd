...
@export var max_accel:float = 60
@export var max_deccel:float = 50
@export var dash_boost:float = 600
@export var dash_hangtime:float = .5

@export var th:float = .6 # time of jump, from start to landing
@export var h:float = 450 # max height of jump
@export var v_limit:float =900 #max vertical speed
@export var delta_factor:float = 2
@export var damping_speed:float = 300 # +y speed added if jump button released before jump is over.

@export var max_air_jumps = 0 # allows for double jump (and triple jump, etc)
@export var inertia_factor:float = .1
@export var air_accel_factor:float = .2 # factor applied to acceleration while on air
@export var coyote_time:float = 0.2 # time after leaving a platform that the player can still jump
@export var jump_buffer:float = 0.2 # time before landing that a jump command will stil take place after landing
@export var hangtime:float = 0.1 # time before gravity starts being applied when player reaches zenith of jump
@export var climb_timeout:float = 1 # time before gravity starts being applied when player reaches zenith of jump
@export var allow_wall_jump:bool = false
@export var wall_jump_x:float=300
@export var wall_control_timeout:float=.3
@export var g_factor:float = 1

@export var xsm_path:NodePath
...
@onready var climb_reload_timer:Timer = $climb_reload_timer
...
#		sfx_jump.play()
		...
		get_tree().create_timer(jump_buffer).timeout.connect(reset_jump_buffer)
...
func do_dash(direction:Vector2 = Vector2.ZERO)->void:
	...
	if not player.is_on_floor():
		...
		Logger.info ("dash speed trimmed.")
