...
#		pass
	if owner.velocity.y>0.01 and !owner.is_on_floor() and owner.controller.coyote_time>0 and owner.controller.landed:
		...
		get_tree().create_timer(owner.controller.coyote_time).timeout.connect( owner.controller.reset_coyote_time)
...
func _on_update(_delta: float) -> void:
	...
	if owner.is_on_wall() and not owner.is_on_floor()\
		and owner.dimension == Events.Dimension.MATERIAL and owner.controller.can_climb:
			change_state("climb")
			return
